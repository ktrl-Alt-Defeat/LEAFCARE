...
export const CropGrid: React.FC<CropGridProps> = ({ selectedCrops, onToggleCrop }) => {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {status === 'error' ? (
        <div className="flex flex-col items-center gap-2 rounded-3xl border border-dashed border-red-300 bg-red-50/60 px-6 py-14 text-center">
        ...
        <div className="grid grid-cols-3 gap-2.5 xs:gap-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-8">
          {Array.from({ length: 8 }, (_, index) => (
            <div key={index} className="aspect-square animate-pulse rounded-2xl bg-slate-200/60" />
          ))}
        </div>
